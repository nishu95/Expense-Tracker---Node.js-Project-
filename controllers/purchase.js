const Razorpay = require('razorpay');
const Order = require('../models/order');
require('dotenv').config();    

const purchasepremium = async (req,res) => {
    try{
        console.log("inside purchase premium controller")
        var rzp = new Razorpay({
            key_id: process.env.RAZORPAY_KEY_ID,
            key_secret: process.env.RAZORPAY_KEY_SECRET
        });
       

        const amount = 2500;

        rzp.orders.create({amount,currency: "INR"}, (err, order) => {    // creatin razor pay order (here 'orders' is not name of table but a razorpay function)
            if(err){
                throw new Error(JSON.stringify(err));
            }
            req.user.createOrder({orderid: order.id,status:'PENDING'})
                .then(() => {
                    return res.status(201).json({order , key_id : rzp.key_id});
                })
                .catch(err => {throw new Error(err)})
        });

    }catch(err){
        console.error(err);
        res.status(403).json({message:'soething went wrong',error:err});
    }
}

const updateTransactionStatus = async (req, res) => {
    try{
        
        if(req.body.clickStatus === "payment_success"){
            const {payment_id,order_id} = req.body;
            const order = await Order.findOne({where:{orderid: order_id}})
            const promise1 = order.update({paymentid:payment_id,status:"SUCCESSFUL"});
            const promise2 = req.user.update({ispremiumuser: true});

            Promise.all([promise1, promise2]).then(()=>{
                res.status(202).json({success:true,message:"transaction successful"});
            }).catch(err => {throw new Error(err);})
        }
        if(req.body.clickStatus === 'payment_failed'){
            const {payment_id,order_id} = req.body;
            const order = await Order.findOne({where:{orderid: order_id}})
            const promise1 = order.update({paymentid:payment_id,status:"FAILED"});
            res.status(200).json({success:false, message:"transaction failed"});
        }
        
        
    }catch(err){
        console.log(err);
        res.status(401).json({message: err.message});
    }
    
}


module.exports = {purchasepremium,updateTransactionStatus};